...
var swqExport = (function(root) {
    ...
    function parseMultiAnswer(value) {
        ...
        return s.split(',').map(function(part) { return part.trim(); });
    }
    ...
    Quiz.prototype._parseQuestions = function() {
        ...
        if (this.options.questions && Array.isArray(this.options.questions)) {
            ...
            for (var j = 0; j < questionElements.length; j++) {
                ...
                var raw = { id: qEl.dataset.swqQuestionId, element: qEl };
    ...
    Quiz.prototype._setupUI = function() {
        ...
        var needsControls = !this.ui.nextBtn || (this.config.allowBack && !this.ui.prevBtn) ||
            (this.config.allowSkip && !this.ui.skipBtn);
    ...
    Quiz.prototype._startQuiz = function() {
        ...
        if (this.config.timer > 0) {
            ...
            var self = this;
            this.state.timerId = setInterval(function() { self._updateTimer(); }, 1000);
        }
    ...
    Quiz.prototype._renderResults = function(results) {
        ...
        for (var i = 0; i < this.questions.length; i++) {
            ...
            yourAnswer.textContent = (answerData && answerData.skipped) || !answerData
                ? 'Your answer: Skipped'
                : 'Your answer: ' + formatAnswer(answerData.answer);
    ...
    Quiz.prototype._isAnswerCorrect = function(question, userAnswer) {
        ...
        switch (question.type) {
            case 'select-multiple': {
                var expected = parseMultiAnswer(question.answer)
                    .map(function(s) { return String(s).trim(); })
                    .sort();
                var submitted = (Array.isArray(userAnswer) ? userAnswer.slice() : [userAnswer])
                    .map(function(s) { return String(s).trim(); })
                    .sort();
    ...
    Quiz.prototype._updateControls = function() {
        ...
        if (this.ui.nextBtn) {
            ...
            if (this._generated.next) {
                this.ui.nextBtn.textContent = alreadyGraded
                    ? 'Continue'
                    : (index === this.questions.length - 1 ? 'Finish' : 'Next');
            }
        }
    };
    ...
    Quiz.prototype._handleNext = function() {
        ...
        this.state.userAnswers.set(index, { answer: answer, isCorrect: isCorrect });
    ...
    Quiz.prototype._handleSkip = function() {
        ...
        this.state.drafts.delete(index);
        this.state.userAnswers.set(index, { answer: null, isCorrect: false, skipped: true });
        this._advance();
    };
